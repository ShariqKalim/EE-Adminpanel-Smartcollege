...
const AE_TASK_PANEL = () => {
  ...
  const onFormSubmit = async (e) => {
    ...
    try {
      await axios.post(process.env.React_App_API_AE_TASK_PANEL, card);
  ...
  useEffect(() => {
    ...
    const getApiData = async () => {
      try {
        const res = await axios.get(process.env.React_App_API_AE_TASK_PANEL);
        setInfo(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  const handleDelete = async (_id) => {
    await axios.delete(process.env.React_App_API_AE_TASK_PANEL + "/" + _id);
